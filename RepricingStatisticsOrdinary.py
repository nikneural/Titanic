...
class RepricingStatisticsOrdinary:
    def __init__(self, df_cp: pd.DataFrame, target_product: str, main_company: str,
    ...
    def generate_plots(self, allow_plot: bool = True):
        ...
        self.mean_profit = self.plot_data(self.total_df.loc[:, idx[self.main_company, ['profit']]],
                                          title=f'Средняя прибыль . MA({self.period})',
                                          legend=['Ordinary price'])
    ...
    def calculate_value(interval: pd.DataFrame, m_orders: str, price: str,
        ...
        summary = {'from': [interval.index[0]], 'to': [interval.index[-1]],
                   ...
                   'profit_mean': [real_profit.mean()], 'interval': [desc]}
    ...
    def summary_calculation(self) -> pd.DataFrame:
        ...
        full = pd.concat([train_stat, test_stat], ignore_index=True)
        return full
